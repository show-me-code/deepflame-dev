...
namespace Foam
{
    defineTypeNameAndDebug(hexRef4Axi, 0);
    addToRunTimeSelectionTable(hexRef, hexRef4Axi, mesh);
    addToRunTimeSelectionTable(hexRef, hexRef4Axi, levelsHist);
    addToRunTimeSelectionTable(hexRef, hexRef4Axi, levels);
}
...
Foam::label Foam::hexRef4Axi::storeMidPointInfo
(
    const labelListList& cellAnchorPoints,
    const labelListList& cellAddedCells,
...
void Foam::hexRef4Axi::createInternalFaces
(
    const labelListList& cellAnchorPoints,
    const labelListList& cellAddedCells,
    const labelList& cellMidPoint,
...
Foam::hexRef4Axi::hexRef4Axi(const polyMesh& mesh, const bool readHistory)
:
...
Foam::labelListList Foam::hexRef4Axi::setRefinement
(
...
//        if (isA<wedgePolyPatch>(mesh_.boundaryMesh()[patchID])
    ...
    if (debug)
    {
        ...
        Pout<< "hexRef4Axi::setRefinement : Dumping " << splitCells.size()
    ...
    if (debug)
    {
        ...
        Pout<< "hexRef4Axi::setRefinement : Dumping " << splitFaces.size()
    ...
    if (debug)
    {
        Pout<< "hexRef4Axi::setRefinement : Splitting faces" << endl;
    }
...
Foam::labelList Foam::hexRef4Axi::selectUnrefineElems
(
...
Foam::labelList Foam::hexRef4Axi::consistentUnrefinement
(
...
) const
{
    if (debug)
    {
        Pout<< "hexRef4Axi::consistentUnrefinement :"
    ...
    // For hexRef4Axi, unrefinement is based on edges
...
void Foam::hexRef4Axi::calcFaceToSplitPoint
(
...
)
{
    ...
    // For hexRef4Axi, the split elems are edges
...
Foam::labelList Foam::hexRef4Axi::getSplitElems() const
{
    if (debug)
    {
        checkRefinementLevels(-1, labelList(0));
...
void Foam::hexRef4Axi::setUnrefinement
(
